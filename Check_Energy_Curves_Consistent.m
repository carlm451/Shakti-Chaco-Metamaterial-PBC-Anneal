...
energy_list = energy_data(:,2);  %% this should be sum all bonds 

k2_current = energy_data(:,3);

energy_square_list = energy_data(:,4);  %% this should be avg of a square 

energy_diamond_list = energy_data(:,5);  %% this should be avg of a triangle 

energy_list_from_units = Lx*Ly*energy_square_list + 2*Lx*Ly*energy_diamond_list; 

% SUMMING ALL UNITS SHOULD MATCH SUMMING ALL BONDS.  LOOKS LIKE I DID IT
% CORRECTLY AND EVERYTHING MATCHES ... .
...
plot(k2_current(index_list)/k1,energy_list(index_list),'-','LineWidth',2,'Color',colors(i,:)); hold on;
plot(k2_current(index_list)/k1,energy_list_from_units(index_list),':','LineWidth',2,'Color',[0 1 1]); hold on;
